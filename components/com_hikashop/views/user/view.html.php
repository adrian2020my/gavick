...
class userViewUser extends HikaShopView {
	...
	function cpanel(){
		...
		$cpanel_name = JText::_('CUSTOMER_ACCOUNT');
		if (empty($cpanel_name)) {
			$cpanel_name = $app->getCfg('sitename');
		}
		elseif ($app->getCfg('sitename_pagetitles', 0) == 1) {
			$cpanel_name = JText::sprintf('JPAGETITLE', $app->getCfg('sitename'), $cpanel_name);
		}
		elseif ($app->getCfg('sitename_pagetitles', 0) == 2) {
			$cpanel_name = JText::sprintf('JPAGETITLE', $cpanel_name, $app->getCfg('sitename'));
		}
		$document->setTitle( strip_tags($cpanel_name) );
	}
